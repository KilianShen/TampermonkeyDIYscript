...
// @name         DIYscript
// @version      0.0.4
// @description  去除知乎回答赞赏；去除B站屏蔽广告插件提示与卡片；去除虎扑游戏广告；IT之家资讯移动端网页跳转pc页面；CSDN外链自动跳转；
// @author       KilianShen
...
(function () {
  ...
  const websites = [
    ...
    {
      match: '*://m.ithome.com/*',
      condition: !/Android|webOS|iPhone|iPod|BlackBerry/i.test(navigator.userAgent),
      js: () => {
  ...
  for (const website of websites) {
    if (isMatch(website.match) && website.condition) {
      website.js();
    }
  }
