...
// @name         DIYscript
// @version      0.0.3
// @description  去除知乎回答赞赏；去除B站屏蔽广告插件提示与卡片；去除虎扑游戏广告；IT之家资讯移动端网页跳转pc页面；
// @author       KilianShen
...
;(function () {
  ...
  const websites = [
    ...
    },
    {
      ...
      js: () => {
        GM_addStyle(
          '.index_game-center-entrance-container-title__BNope {display:none;}'
        )
        const gameAbSwiper = document.getElementById(
          'game-center-entrance-container'
        )
    ...
    {
      match: '*://m.ithome.com/*',
      condition: !/Android|webOS|iPhone|iPod|BlackBerry/i.test(
        navigator.userAgent
      ),
      js: () => {
  ...
  for (const website of websites) {
    if (isMatch(website.match, location.href) && website.condition) {
      website.js()
    }
  }

  /** 检测当前网址与指定规则是否匹配 */
  function isMatch(originalPattern, link) {
    return link.includes(originalPattern.replaceAll('*', ''))
  }
})()
